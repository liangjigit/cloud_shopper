  ...
  <div class="container_filtering">
    <div class="searchBox">
      <form method="post" class="search" id="search" @submit.prevent="submit">
        ...
        <input type="text" id="searchBar" results="5" placeholder="输入会员手机号码/会员卡号" class="searchBar" v-model="searchBar" />
        <!-- 右上角筛选框 -->
        ...
        <ul class="memberList" id="memberList" style="margin: 0">
          ...
          <template v-else-if="memberstate == 2">
            <li v-for="(item, id) in myClient" :memberId="item.id" :key="id">
              <label ref="labelC" v-if="memberstate == 2" :for="item.id" style="margin-left: 0.3rem" @click="assignMembers(id)">
                <img :src="imgcheck" />
              </label>
              <div class="memberImg">
          ...
          <template v-else-if="memberstate == 3">
            <li v-for="(item, id) in myClient" :memberId="item.id" :key="id">
              <label ref="labelC" v-if="memberstate == 3" :for="item.id" style="margin-left: 0.3rem" @click="assignMembers(id)">
                <img :src="imgcheck" />
              </label>
              <div class="memberImg">
                <img :src="item.avatar" alt class="memberHead" />
      ...
      <div class="app-menu" @click="toggleClassMenu">
        <div class="menu_inside">
          ...
          <ul class="filterBox shopGuide" v-show="isshopGuide">
            <li v-for="item in shopGuide" :type="item.guideId" :class="shopGuides == item.guideId ? 'tabin_in' : ''" :key="item.guideId" @click="shopGuides = item.guideId">
              {{ item.name }}
            </li>
          </ul>
          ...
          <ul class="filterBox last">
            <li class="lastday" :key="index" :type="item.type" @click="chooseLastTime(index);lastTime = item.type" v-for="(item, index) in lastTimeList" :class="lastTime == item.type ? 'tabin_in' : ''">
              {{ item.txt }}
            </li>
          </ul>
    ...
    */
    ...
    changeFixed(clientHeight) {
      ...
      if (_this.memberstate == "2" || _this.memberstate == "3") {
        if (document.querySelector('.searchBox') && document.querySelector('.check') && document.querySelector('.determine')) {
