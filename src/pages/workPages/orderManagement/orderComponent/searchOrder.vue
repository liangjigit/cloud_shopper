      ...
      <ul class="orderList">
        <li v-for="item in myOrderList" :key="item.orderId" @click="orderDetail($event)" :id="item.orderId">
          ...
          <div class="orderNumberLine">
            ...
            <span class="orderStatus">{{ item.orderType }}</span>
          </div>
          <!-- 商品详情 -->
          <div class="order_goods" v-for="(itemChild, indexChild) in item.orderMxes" :key="indexChild">
      ...
      allOrderMoreData() {
        ...
        that.$api.get(
          ...
          (res) => {
            ...
            this.$emit('getOrderType', res.data.orderType)
