<template>
  <keep-alive>
    <scroll ref="pullrefresh" :pullup="isShow" @scrollToEnd="loadBottom" :data="orderList"
      ...
      <ul class="orderList">
        <li v-for="(item,index0) in myOrderList" :key="index0" @click="orderDetail($event)" :id="item.orderId">
          <!-- 订单号/状态 -->
          <div class="orderNumberLine">
            <span class="orderNum">订单号：{{ item.orderNum }}</span><span class="orderStatus">{{ item.orderType }}</span>
          </div>
          <!-- 商品详情 -->
          <div class="order_goods" v-for="(itemChild,index1) in item.orderMxes" :key="index1">
